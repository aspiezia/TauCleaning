...
IsoDepositVetoFactory::make(const char *string, reco::isodeposit::EventDependentAbsVeto *&evdep) {
    ...
    static boost::regex 
        ...
        otherCand("^(.*?):(.*)"),
        boostedTauDR("BoostedTauByDR\\((\\w+:?\\w*:?\\w*),\\s*(\\d+\\.?|\\d*\\.\\d*)\\)"),
        boostedTauConstituentsDR("BoostedTauConstituentsDeltaRVeto\\((\\w+:?\\w*:?\\w*),\\s*(\\d+\\.?|\\d*\\.\\d*),\\s*(\\w+:?\\w*:?\\w*),\\s*(\\d+\\.?|\\d*\\.\\d*)\\)"),
        boostedTau("^(.*?):(.*)"),
        number("^(\\d+\\.?|\\d*\\.\\d*)$");
    boost::cmatch match;
    ...
    } else if (regex_match(string, match, boostedTauDR)) {
        BoostedTauDeltaRVeto *ret = new BoostedTauDeltaRVeto(edm::InputTag(match[1]), 
							     atof(match[2].first));
        evdep = ret;
        return ret;    
    } else if (regex_match(string, match, boostedTauConstituentsDR)) {
        BoostedTauConstituentsDeltaRVeto *ret = new BoostedTauConstituentsDeltaRVeto(Direction(), 
						   edm::InputTag(match[1]), atof(match[2].first),
						   edm::InputTag(match[3]), atof(match[4].first));
        evdep = ret;
        return ret;
    } else if (regex_match(string, match, boostedTau)) {
        BoostedTauVeto *ret = new BoostedTauVeto(edm::InputTag(match[1]), 
                                                           make(match[2].first));
        evdep = ret;
        return ret;
